...
export const automationJob = task({
  ...
  run: async (payload: AutomationPayload, { ctx }) => {
    ...
    // Process each person independently, including fetching common data
    const results = await Promise.all(
      pcoPersonIds.map(async (pcoPersonId) => {
        let currentMemberRecordId: number | null = null;
        try {
          // Fetch automation details (moved inside person loop to prevent global failure)
    ...
    const summary = results.reduce(
      ...
      { total: 0, completed: 0, failed: 0, canceled: 0, skipped: 0 } as Record<
        string,
        number
      >,
    );
