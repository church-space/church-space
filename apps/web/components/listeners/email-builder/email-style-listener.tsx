...
export default function EmailStyleListener({
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`email-style-changes-${emailId}`)
      .on(
        "postgres_changes",
        {
          event: "UPDATE",
      ...
      .subscribe((status) => {
        ...
        if (status === "SUBSCRIBED") {
          console.log(
            `Subscribed to email style changes for email ID: ${emailId}`,
          );
        } else if (status === "CHANNEL_ERROR") {
          console.error(
            `Failed to subscribe to email style changes for email ID: ${emailId}`,
          );
        }
      });
